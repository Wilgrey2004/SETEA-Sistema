...
namespace SETEA_Sistema
{
    public partial class Gestion : MaterialForm
    {
        ...
        {
            // Configuración del SaveFileDialog con el nombre de archivo
            SaveFileDialog saveFile = new SaveFileDialog();
            saveFile.FileName = $"Reporte Usuarios de la fecha y hora {DateTime.Now.ToString("ddMMyyyyHHmm")}.pdf";

            // Se muestra el diálogo y se valida que el usuario haya aceptado
            if (saveFile.ShowDialog() == DialogResult.OK)
            {
                string filas = "";

                // Se recorren las filas del DataGridView y se arma la cadena HTML
                foreach (DataGridViewRow row in MyDataUsers.Rows)
                {
                    filas += "<tr>";
                    filas += $"<td style=\"padding: 8px; border-bottom: 2px solid #ddd; width: 20%\"> {row.Cells[0].Value?.ToString()} </td>";
                    filas += $"<td style=\"padding: 8px; border-bottom: 2px solid #ddd; width: 20%\"> {row.Cells[1].Value?.ToString()} </td>";
                    filas += $"<td style=\"padding: 8px; border-bottom: 2px solid #ddd; width: 20%\"> {row.Cells[2].Value?.ToString()} </td>";
                    filas += $"<td style=\"padding: 8px; border-bottom: 2px solid #ddd; width: 20%\"> {row.Cells[3].Value?.ToString()} </td>";
                    filas += $"<td style=\"padding: 8px; border-bottom: 2px solid #ddd; width: 20%\"> {row.Cells[4].Value?.ToString()} </td>";
                    filas += $"<td style=\"padding: 8px; border-bottom: 2px solid #ddd; width: 20%\"> {row.Cells[5].Value?.ToString()} </td>";
                    filas += $"<td style=\"padding: 8px; border-bottom: 2px solid #ddd; width: 20%\"> {row.Cells[6].Value?.ToString()} </td>";
                    filas += "</tr>";
                }

                // Se obtiene la plantilla HTML y se reemplazan los marcadores por los datos correspondientes
                string paginaHtml_text = Properties.Resources.PlantillaReporteUsuarios.ToString();
                ...
                using (FileStream stream = new FileStream(saveFile.FileName, FileMode.Create))
                {
                    ...
                    // Se agrega un espacio o encabezado vacío (puedes personalizarlo si lo deseas)
                    pdfDoc.Add(new Phrase(""));

                    // Se procesa el HTML para agregar el contenido al PDF
